...
if (typeof window !== 'undefined' && !isDevelopment) {
  try {
    ...
    console.error('인증 지속성 설정 중 예외 발생:', error);
  }
}
...
if (typeof window !== 'undefined') {
  try {
    ...
    auth.onAuthStateChanged((user) => {
        ...
        if (!isDevelopment) {
          try {
            ...
            user.getIdToken(true).then((token) => {
              ...
              console.error('초기 토큰 갱신 오류:', error);
            });
            
            // 컴포넌트 언마운트 시 인터벌 정리
            return () => clearInterval(tokenRefreshInterval);
          } catch (error) {
            console.error('토큰 갱신 설정 중 오류:', error);
          }
        }
      } else {
      ...
      console.error('인증 상태 변경 감지 중 오류:', error);
    });
  } catch (error) {
    console.error('인증 상태 변경 리스너 설정 중 오류:', error);
  }
}
