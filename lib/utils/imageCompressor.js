...
export const compressImage = (file, options = {}, progressCallback = () => {}) => {
  const {
    ...
    maxHeight = 1000,
    quality = 0.5,
    maxSize = 800000, // 800KB (Firestore 제한 1MB보다 작게)
...
function applyAdditionalCompression(canvas, img, maxSize) {
  ...
  // 더 강한 압축 적용 (품질 30%)
  let compressedDataUrl = canvas.toDataURL('image/jpeg', 0.3);
...
function createFallbackImage(img) {
  ...
  // 이미지를 중앙에 배치
  const size = Math.min(500, img.width, img.height);
  const offsetX = (500 - size) / 2;
  const offsetY = (500 - size) / 2;
