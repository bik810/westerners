  ...
  try {
    ...
    if (isDevelopment) {
      ...
      try {
        ...
        console.log('개발 환경 - 모의 사용자 정보 저장:', user.uid, userData);
        console.log('개발 환경 - 이메일 인증 메일 전송 (모의):', email);
        
        // 개발 환경에서는 이메일 인증 메일 전송을 시뮬레이션
        await mockSendEmailVerification(user);
        
        // 개발 환경에서 비밀번호 재설정 이메일도 함께 전송
    ...
    console.log('Firestore 저장 성공, 이메일 인증 메일 전송 중:', email);
    
    // 이메일 인증 메일 전송
    await sendEmailVerification(user);
    
    console.log('이메일 인증 메일 전송 성공, 비밀번호 설정 이메일 전송 중');
    
    // 비밀번호 설정 이메일도 함께 전송
    await sendPasswordResetEmail(auth, email);
    
    console.log('비밀번호 설정 이메일 전송 성공, 관리자 세션 유지를 위해 로그아웃');
