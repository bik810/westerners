  ...
  try {
    ...
    if (isDevelopment) {
      ...
      try {
        ...
        // 개발 환경에서 비밀번호 재설정 이메일 전송
        console.log('개발 환경 - 비밀번호 설정 이메일 전송 (모의):', email);
    ...
    console.log('Firestore 저장 성공, 비밀번호 설정 이메일 전송 중');
    
    // 비밀번호 설정 이메일 전송
    await sendPasswordResetEmail(auth, email);
    
    console.log('비밀번호 설정 이메일 전송 성공, 관리자 세션 유지를 위해 로그아웃 방지');
    
    // 관리자 계정으로 다시 로그인
    if (currentAdmin) {
      console.log('관리자 계정으로 다시 로그인 시도:', currentAdmin.email);
      
      try {
        // 개발 환경에서는 모의 로그인 함수 사용
        if (isDevelopment) {
          await mockSignIn(null, currentAdmin.email, 'admin-password');
        } else {
          // 실제 환경에서는 signInWithCustomToken 등 다른 방법이 필요할 수 있음
          // 여기서는 관리자 로그인을 복원하지 않고 알림만 표시
          console.log('관리자 세션 변경됨: 페이지를 새로고침하면 관리자로 다시 로그인해야 할 수 있습니다.');
        }
      } catch (reloginError) {
        console.error('관리자 계정 재로그인 실패:', reloginError);
        console.log('관리자 계정으로 다시 로그인이 필요할 수 있습니다.');
      }
    }
