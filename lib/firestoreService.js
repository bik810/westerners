...
export const loginUser = async (email, password) => {
  try {
    if (isDevelopment) {
      ...
      try {
        const result = await mockSignIn(mockAuthModule.mockAuth, email, password);
        console.log('개발 환경 - 모의 로그인 성공:', result);
        return result;
      } catch (mockError) {
        console.error('개발 환경 - 모의 로그인 오류:', mockError);
        throw mockError;
      }
    }

    console.log('프로덕션 환경 - 실제 로그인 시도:', email);
    try {
      const result = await signInWithEmailAndPassword(auth, email, password);
      console.log('프로덕션 환경 - 로그인 성공');
      return result;
    } catch (authError) {
      console.error('프로덕션 환경 - 로그인 오류:', authError);
      // Firebase 인증 오류 코드 확인
      if (authError.code) {
        console.error('Firebase 인증 오류 코드:', authError.code);
      }
      throw authError;
    }
  } catch (error) {
